# author：yang.ye
# Created Date: 24/10/26
# Last Modified Date: 24/10/29
# Copyright (c) 2024 yang.ye
...
class ChatWidget(QWidget):
    ...
    def listen(self,listen_enable=True):
        ...
        CHUNK = 320  # 每个缓冲区的帧数
        WAVE_OUTPUT_FILENAME = "./voice/output.wav"  # 输出文件名
        VAD_MODE = 1  # 
        self.sleep = True
        ...
        def record_audio(stream, vad, silence_threshold=30):
            frames = []
            no_speak = 0
            if self.sleep:
                RECORD_SECONDS = 1
            ...
            for _ in range(0, int(RATE / CHUNK * RECORD_SECONDS)):
                ...
                if is_speech:
                    ...
                    # print("no speak")
                    no_speak += 1
                    if self.sleep == False and no_speak > (RATE / CHUNK * 5) and len(frames) > (RATE / CHUNK * 3):
                        break
                    if no_speak > (RATE / CHUNK * silence_threshold):
                        self.sleep = True
                        break
            return b''.join(frames)

        vad = webrtcvad.Vad()
        vad.set_mode(VAD_MODE)  # 设置VAD的敏感度

        response_enable = False
        with open_audio_stream() as stream:
            while listen_enable:
                audio_data = record_audio(stream, vad)
                if not audio_data:
                    continue
